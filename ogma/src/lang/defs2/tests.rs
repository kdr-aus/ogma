...
fn fs_structure_building() {
    ...
    assert_eq!(m.keys().cloned().collect::<Vec<_>>(), keys(vec!["bar/foo", "foo", "foo/bar", "foo/foo"]));
    assert_eq!(m.values().map(Vec::len).collect::<Vec<_>>(), vec![2, 2, 1, 2]);

    let x = build_fs_map(Path::new("partitions-structure-tests/test05")).unwrap_err().to_string();
    println!("{x}");
    assert_eq!(&x, "Parsing Error: the partition 'foo' is defined adjacent and as a directory\n");
}
